...
export default function UploadDocAIGen() {
    ...
    const interval = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        const mockGen: GeneratedQuestion[] = [
          {
            id: 'G1', content: 'What is the time complexity of BFS on an adjacency list?',
            ...
            difficulty: 0.55, topic: 'Graph Traversal', approved: null, editing: false,
          },
          {
            id: 'G2', content: 'Kruskal\'s algorithm always produces a minimum spanning tree for any connected weighted graph.',
            type: 'true_false', tfAnswer: true,
            difficulty: 0.40, topic: 'Minimum Spanning Tree', approved: null, editing: false,
          },
          {
            id: 'G3', content: 'Which data structure is used in Dijkstra\'s algorithm for optimal performance?',
            type: 'multiple_choice',
            options: [
            ...
            difficulty: 0.62, topic: 'Shortest Path', approved: null, editing: false,
          },
          {
            id: 'G4', content: 'A topological sort is possible only on DAGs (Directed Acyclic Graphs).',
            type: 'true_false', tfAnswer: true,
            difficulty: 0.35, topic: 'Graph Properties', approved: null, editing: false,
          },
          {
            id: 'G5', content: 'What is the worst-case time complexity of Bellman-Ford algorithm?',
            type: 'multiple_choice',
            options: [
            ...
            difficulty: 0.72, topic: 'Shortest Path', approved: null, editing: false,
          },
        ];
  ...
  const approveQuestion = (id: string) => {
    setGenerated((prev) => prev.map((q) => (q.id === id ? { ...q, approved: true } : q)));
  };

  const rejectQuestion = (id: string) => {
    setGenerated((prev) => prev.map((q) => (q.id === id ? { ...q, approved: false } : q)));
  };
  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-foreground mb-1">Upload Document & AI Generate</h1>
          <p className="text-muted-foreground">
            Upload course materials and let AI generate exam questions automatically
          </p>
        </div>

        {/* Step Indicator */}
        <div className="flex items-center gap-2 mb-8">
          {(['upload', 'configure', 'generating', 'review'] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`flex items-center justify-center h-8 w-8 rounded-full text-sm font-medium
                ${step === s ? 'bg-primary text-primary-foreground' :
                  (['upload', 'configure', 'generating', 'review'].indexOf(step) > i ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground')}`}
              >
                {['upload', 'configure', 'generating', 'review'].indexOf(step) > i ? (
        ...
        {step === 'upload' && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Upload PDF, DOCX, or TXT files containing course material</CardDescription>
            </CardHeader>
                  ...
                  {files.map((f) => (
                    <div key={f.name} className="flex items-center gap-3 p-3 rounded-lg border">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">{f.size} · {f.type}</p>
                      </div>
                      {uploadProgress >= 100 && <CheckCircle2 className="h-5 w-5 text-green-600" />}
                    </div>
                  ))}
        ...
        {step === 'configure' && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure how AI should generate questions from your documents</CardDescription>
            </CardHeader>
                ...
                <div className="flex gap-4">
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(c) => setQuestionTypes({ ...questionTypes, mc: !!c })}
                    />
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(c) => setQuestionTypes({ ...questionTypes, tf: !!c })}
                    />
        ...
        {step === 'generating' && (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground">{genProgress}% complete</p>
            </CardContent>
          </Card>
        )}
        ...
        {step === 'review' && (
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              <Button variant="outline" onClick={() => { setStep('configure'); setGenerated([]); }} className="gap-2">
                <RefreshCw className="h-4 w-4" /> Regenerate
              </Button>
              <Button onClick={saveApproved} disabled={approvedCount === 0} className="gap-2">
                <Save className="h-4 w-4" /> Save {approvedCount} to Question Bank
              </Button>
            </div>

            {/* Question Cards */}
            {generated.map((q, idx) => (
              <Card key={q.id} className={`${q.approved === false ? 'opacity-50' : ''}`}>
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      {q.approved === null && (
                        <>
                          <Button variant="outline" size="sm" onClick={() => approveQuestion(q.id)} className="gap-1 text-green-600 hover:text-green-700">
                            <CheckCircle2 className="h-4 w-4" /> Approve
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => rejectQuestion(q.id)} className="gap-1 text-red-600 hover:text-red-700">
                ...
                <CardContent>
                      ...
                      {q.options.map((o) => (
                        <div
                          ...
                          className={`flex items-center gap-2 p-2 rounded border text-sm ${
                            o.isCorrect ? 'border-green-500 bg-green-50' : 'border-muted'
                          }`}
                        >
                          <span className="font-medium text-muted-foreground">{o.id}.</span>
                          {o.text}
                          {o.isCorrect && <CheckCircle2 className="h-3.5 w-3.5 text-green-600 ml-auto" />}
                        </div>
                      ))}
                    </div>
                  )}
                  {q.type === 'true_false' && (
                    <div className="flex gap-2">
                      <span className={`px-3 py-1 rounded text-sm ${q.tfAnswer ? 'bg-green-50 text-green-700 font-medium' : 'text-muted-foreground'}`}>True</span>
                      <span className={`px-3 py-1 rounded text-sm ${!q.tfAnswer ? 'bg-green-50 text-green-700 font-medium' : 'text-muted-foreground'}`}>False</span>
                    </div>
                  )}
                </CardContent>
              </Card>
