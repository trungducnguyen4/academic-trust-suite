...
const mockHistory: QuestionMetricHistory[] = [
  {
    id: "Q001",
    content:
      "Define the time complexity of Dijkstra's algorithm using a min-heap.",
    course: "CS301",
    metrics: [
      {
        exam: "Midterm 2024",
    ...
    trend: "degrading",
    recommendation:
      "Difficulty has increased by 31% over 4 exams. Consider revising or replacing this question.",
  },
  {
    id: "Q002",
  ...
  },
  {
    id: "Q003",
    content:
      "A binary search tree always guarantees O(log n) lookup time. True or False?",
    course: "CS301",
    metrics: [
      {
        exam: "Midterm 2024",
    ...
    trend: "degrading",
    recommendation:
      "Discrimination has dropped below 0.25. Question may be too easy or poorly worded. Consider retirement.",
  },
  {
    id: "Q004",
...
const revisionHistory = [
  {
    ...
    user: "Dr. Nguyen",
    detail: 'Tags updated: added "min-heap"',
  },
  {
    date: "2025-11-20",
    question: "Q003",
    ...
    user: "Dr. Nguyen",
    detail: "Added clarification about balanced vs unbalanced BST",
  },
  {
    date: "2025-10-05",
    question: "Q002",
    action: "Difficulty adjusted",
    user: "System",
    detail: "Auto-adjusted from estimated 0.50 to measured 0.55",
  },
  {
    date: "2025-09-18",
    question: "Q004",
    action: "Created",
    user: "Dr. Le",
    detail: "New question added to bank",
  },
  {
    date: "2025-08-01",
    question: "Q001",
    ...
    user: "System",
    detail: "Difficulty drift exceeded threshold (+17%)",
  },
];
...
export default function QuestionHistoryAnalysis() {
  ...
  const [selectedQuestion, setSelectedQuestion] =
    useState<QuestionMetricHistory>(mockHistory[0]);
  const [courseFilter, setCourseFilter] = useState("all");

  const filtered =
    courseFilter === "all"
      ? mockHistory
      : mockHistory.filter((q) => q.course === courseFilter);
  ...
  const barData = {
    ...
    datasets: [
      {
        label: "Correct Rate",
        data: selectedQuestion.metrics.map((m) =>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-muted-foreground">
            Track difficulty drift, psychometric trends, and revision history
            across exam cycles
          </p>
        </div>
                  ...
                  {filtered.map((q) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground mt-1">
                        {q.course} · {q.metrics.length} exams
                      </p>
                    </button>
                  ))}
                </CardContent>
                ...
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base">
                      Psychometric Trends – {selectedQuestion.id}
                    </CardTitle>
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 mt-2">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                navigate(
                                  `${questionEditorPath}?id=${selectedQuestion.id}`,
                                )
                              }
                            >
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">
                  Difficulty Drift Overview
                </CardTitle>
                <CardDescription>
                  Questions whose difficulty has changed significantly across
                  exams
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {mockHistory.map((q) => {
                      ...
                      const discChange = (
                        ((current.discrimination - initial.discrimination) /
                      ...
                      return (
                        <TableRow key={q.id}>
                          ...
                          <TableCell>
                            <div className="space-y-0.5">
                              <span className="text-sm">
                                {initial.difficulty.toFixed(2)}
                              </span>
                              <Progress
                                value={initial.difficulty * 100}
                                className="h-1"
                              />
                            </div>
                          </TableCell>
                          <TableCell>
                            <div className="space-y-0.5">
                              <span className="text-sm">
                                {current.difficulty.toFixed(2)}
                              </span>
                              <Progress
                                value={current.difficulty * 100}
                                className="h-1"
                              />
                            </div>
                          </TableCell>
                          ...
                          <TableCell>
                            <span
                              className={`text-sm ${Number(discChange) < -10 ? "text-red-600" : "text-muted-foreground"}`}
                            >
                              {current.discrimination.toFixed(2)} (
                              {Number(discChange) > 0 ? "+" : ""}
                              {discChange}%)
                            </span>
                          </TableCell>
          ...
          <TabsContent value="history">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  All changes and updates to questions over time
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <div className="flex flex-col items-center">
                        <div
                          className={`h-3 w-3 rounded-full ${
                            entry.action.includes("Flagged")
                              ? "bg-yellow-500"
                              : entry.action.includes("Created")
                                ? "bg-green-500"
                                : entry.action.includes("revised")
                                  ? "bg-blue-500"
                                  : "bg-muted-foreground"
                          }`}
                        />
