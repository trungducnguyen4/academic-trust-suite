...
export default function OfflineExamDownload() {
  ...
  const handleDelete = (pkgId: string) => {
    setPackages((prev) =>
      prev.map((p) =>
        p.id === pkgId
          ? { ...p, status: "available", downloadedAt: undefined }
          : p,
      ),
    );
  };
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-muted-foreground mb-6">
          Download exam packages for offline use. Answers will sync
        ...
        <Alert className="mb-6">
          ...
          <AlertDescription>
            Downloaded packages are encrypted and can only be unlocked at the
            scheduled exam time. Your answers will be stored locally and synced
        ...
        <div className="space-y-4">
          {packages.map((pkg) => (
            <Card
              key={pkg.id}
              className={pkg.status === "expired" ? "opacity-60" : ""}
            >
              <CardContent className="pt-5 pb-5">
                  ...
                  <div className="flex-1">
                    ...
                    {pkg.downloadedAt && (
                      <p className="text-xs text-muted-foreground">
                        Downloaded:{" "}
                        {new Date(pkg.downloadedAt).toLocaleString()}
                      </p>
                    )}
                  </div>
                    ...
                    {pkg.status === "downloaded" && (
                      <div className="flex gap-2">
                        ...
                        <Button
                          size="sm"
                          className="gap-2"
                          onClick={() => navigate("/student/exam-ready")}
                        >
                ...
                {pkg.status === "downloading" && (
                  <div className="mt-3">
                    <Progress
                      value={downloadProgress[pkg.id] || 0}
                      className="h-2"
                    />
                  </div>
                )}
