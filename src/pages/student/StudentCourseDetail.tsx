  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {loading && (
          <Card>
            <CardContent className="py-10 text-center text-muted-foreground">Loading course data...</CardContent>
          </Card>
        )}
        ...
        {!loading && error && (
          <Card>
            <CardContent className="py-10 text-center">
              <p className="font-medium text-foreground">Could not load this course.</p>
              ...
              {sortedExams.length === 0 ? (
                ...
                <div className="space-y-3">
                  {sortedExams.map((exam) => {
                    const canJoin = exam.status === 'PUBLISHED' || exam.status === 'ONGOING';
                    return (
                      <div
                        key={exam.id}
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1">
                              ...
                              {exam.startTime ? format(new Date(exam.startTime), 'MMM d, yyyy HH:mm') : 'Not scheduled'}
                            </span>
                            <span className="flex items-center gap-1">
                              <Clock className="h-4 w-4" />
                              {exam.duration || 0} min
                            </span>
                            <Badge variant="outline">{statusLabel[exam.status] || exam.status}</Badge>
                          </div>
                        </div>

                        {canJoin ? (
                          <Button asChild>
                            <Link to={`/student/exam-ready?examId=${exam.id}`}>Open Exam</Link>
                          </Button>
                        ) : (
